...
import com.exampl3.flashlight.model.alarmReceiwer.NotificationBuilder
...
class AlarmReceiwer: BroadcastReceiver() {
    ...
    @Inject
    lateinit var modelFlashLight: AlarmManagerImp
    @Inject
    lateinit var ringtone: Ringtone
    @Inject
    lateinit var providesAlarmPush : NotificationManager
    @Inject
        ...
        when (intent.action) {
            ...
            Const.keyIntentCallBackReady -> {
               ...
               // alarmPush(context).cancel(item.id!!)
                providesAlarmPush.cancel(item.id!!)
               ...
               // val time = calendarZero.timeInMillis + 600000
                ...
                when(item.interval){
                    Const.alarmOne->{
                        ...
                        modelFlashLight.alarmInsert(newItem, Const.alarmOne)
                    } else->{
                        val newItemFals = item.copy(id = item.id?.plus(1000), interval = Const.alarmRepeat)
                    modelFlashLight.alarmInsert(newItemFals, Const.alarmOne)
                    }
                }
...
//    }
    ...
    private fun insertAlarm(item: Item, context: Context,intervalTime: Long, intervalString:String){
        ...
        modelFlashLight.alarmInsert(newItem, item.interval)
